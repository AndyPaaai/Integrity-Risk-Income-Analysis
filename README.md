...
│   └── Supplementary Table S9.csv             # Weighting sensitivity analysis (equal-weight composite)
│
...
- **Weighting sensitivity analysis** comparing an alternative equal-weight composite (1/9 each across all nine original indicators) against the published AHP-weighted Overall score across composite agreement, country-level Spearman correlations, weighted country regression, and multilevel model specifications (Supplementary Table S9)
...
- `outputs_revised/supplementary/` — Supplementary Tables S1–S9 as sectioned CSV files
...
For the versioned repository deposit, the final Supplementary Tables S1–S9 and Supplementary Datasets S1–S2 referenced by the manuscript are retained in `supplementary material/`. The `analysis_session_info.txt` produced by the same exact-reproduction run is also retained at the repository root.
...
| **Supplementary Table S9** | Sensitivity of IRIS Overall-based findings to an alternative equal-weight indicator specification (composite agreement, country-level associations, weighted country regression, and multilevel model fit and fixed effects) |
